  ...
  const updateChat = async (chatId: string, data: Partial<Omit<ChatData, 'id'>>) => {
    ...
    if (dbConnected) {
      try {
        ...
        const result = await db.updateChat(chatId, dbData);
        
        // If we have flashcards, save them too
        if (data.flashcards) {
          const flashcardResult = await db.saveFlashcards(chatId, data.flashcards);
          if (!flashcardResult) {
            console.error('Failed to save flashcards to database');
          }
        }
      } catch (error) {
